...
export default function AdminMaster() {
  ...
  const baixarBackup = (empresa: Empresa) => {
    if (!empresa?.id) return;
    const confirmar = confirm(`Gerar backup completo da empresa ${empresa.nome}?`);
    if (!confirmar) return;
    window.location.href = `/api/master/empresas/${empresa.id}/backup`;
  };
                  ...
                  <tr key={empresa.id} className="border-t border-white/10 align-middle text-slate-100 hover:bg-white/[0.03]">
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap justify-center gap-2">
                        ...
                        <Button className={BOTAO_SECUNDARIO} size="sm" variant="outline" onClick={() => baixarBackup(empresa)} title="Backup"><Download className="h-4 w-4" /></Button>
