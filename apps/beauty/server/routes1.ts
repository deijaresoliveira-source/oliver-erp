...
export function registerApiRoutes(app: Express) {
  ...
  async function ensureMultiEmpresaTables() {
    await execute(`
      CREATE TABLE IF NOT EXISTS empresas (
        ...
        sistema_tipo VARCHAR(30) NOT NULL DEFAULT 'beauty',
    ...
    await execute("ALTER TABLE empresas ADD COLUMN sistema_tipo VARCHAR(30) NOT NULL DEFAULT 'beauty'").catch(() => undefined);
    ...
    if (!empresaRows[0]) {
      await execute(`
        INSERT INTO empresas (nome, marca, slug, sistema_tipo, email, plano, ativo, ativa_ate, dias_aviso, logo_dbo, logo_empresa, logo_menu, fundo_login)
        VALUES ('LetsBarbearia', 'LetsBarbearia', 'letsbarbearia', 'beauty', 'deijares@gmail.com', 'Teste', 1, DATE_ADD(CURDATE(), INTERVAL 30 DAY), 7, '/logo-dbo.png', '/logo-empresa.png', '/logo-menu.png', '/background-login.jpg')
      `).catch(() => undefined);
    }
  ...
  function mapEmpresaMaster(e: any) {
    ...
    return {
      ...
      nome: e.nome || '',
      slug: e.slug || '',
      sistemaTipo: e.sistema_tipo || 'beauty',
      sistema_tipo: e.sistema_tipo || 'beauty',
      sistemaNome: e.sistema_tipo === 'fit' ? 'Oliver Fit' : e.sistema_tipo === 'clinic' ? 'Oliver Clinic' : 'Oliver Beauty',
      email: e.email || '',
      telefone: e.telefone || '',
      plano: e.plano || 'Teste',
  ...
  app.post("/api/publico/cadastrar-empresa", async (req, res) => {
    try {
      ...
      const sistemaTipo = String(b.sistemaTipo || b.sistema_tipo || (segmento.toLowerCase().includes('academ') ? 'fit' : segmento.toLowerCase().includes('clinic') ? 'clinic' : 'beauty')).trim().toLowerCase();
      ...
      // A restrição correta é por empresa_id + email, definida na tabela usuarios.
      ...
      const result = await execute(
        `INSERT INTO empresas
         (nome, marca, slug, sistema_tipo, email, telefone, documento, endereco, observacao, plano, ativo, ativa_ate, dias_aviso, bloqueio_motivo, logo_dbo, logo_empresa, logo_menu, fundo_login, cor_primaria)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Teste', 1, DATE_ADD(CURDATE(), INTERVAL 7 DAY), 2, NULL, '/logo-dbo.png', NULL, NULL, NULL, '#f6b21a')`,
        [
          empresaNome,
          empresaNome,
          slug,
          ['beauty', 'fit', 'clinic'].includes(sistemaTipo) ? sistemaTipo : 'beauty',
  ...
  app.post('/api/master/empresas', async (req, res) => {
    try {
      ...
      const sistemaTipo = String(b.sistemaTipo || b.sistema_tipo || 'beauty').trim().toLowerCase();
      if (!nome) throw new Error('Informe o nome da empresa.');
      if (!slug) throw new Error('Informe o slug da empresa.');

      const result = await execute(
        `INSERT INTO empresas
         (nome, marca, slug, sistema_tipo, email, telefone, plano, ativo, ativa_ate, dias_aviso, bloqueio_motivo, logo_dbo, logo_empresa, logo_menu, fundo_login, cor_primaria)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          nome,
          b.marca || nome,
  ...
  app.put('/api/master/empresas/:id', async (req, res) => {
    try {
      ...
      await execute(
        `UPDATE empresas SET
          nome = ?, marca = ?, slug = ?, sistema_tipo = ?, email = ?, telefone = ?, plano = ?, ativo = ?, ativa_ate = ?, dias_aviso = ?, bloqueio_motivo = ?,
